...
def plot_performance_violin(labels, headline=None):
    ...
    # The average-speedup takeaway lives in the chart title, so it stays
    # up to date everywhere the SVG is hotlinked (e.g. complex-bessel README).
    if headline:
        title = f"Evaluation Time  |  {headline}"
    else:
        title = "Evaluation Time (lower is better)"
...
def _compute_speedups():
    """Average speedup of Rust vs Fortran/SciPy from the bench results.

    Returns (pct_fortran, mult_scipy); either is None when the bench data
    for that implementation is missing.
    """
    bench_rust = load_json("rust_bench.json")
    bench_fortran = load_json("fortran_bench.json")
    bench_scipy = load_json("scipy_bench.json")
    ...
    pct_fortran = (np.mean(ratios_fortran) - 1) * 100 if ratios_fortran else None
    mult_scipy = np.mean(ratios_scipy) if ratios_scipy else None
    return pct_fortran, mult_scipy


def _update_bench_speedup(readme_path, pct_fortran, mult_scipy):
    """Update the average-speedup sentence in README."""
    parts = []
    if pct_fortran is not None:
        parts.append(f"**{pct_fortran:.0f}% faster** than Fortran")
    if mult_scipy is not None:
        parts.append(f"**{mult_scipy:.0f}× faster** than SciPy")
...
def main():
    ...
    pct_fortran, mult_scipy = _compute_speedups()
    headline = None
    if pct_fortran is not None and mult_scipy is not None:
        headline = (
            f"{pct_fortran:.0f}% faster than Fortran, "
            f"{mult_scipy:.0f}× faster than SciPy"
        )

    print("Generating dashboard SVGs...", flush=True)
    plot_accuracy_violin(labels)
    plot_rust_vs_fortran()
    plot_performance_violin(labels, headline)
    plot_summary_table(versions)

    readme_path = os.path.join(os.path.dirname(__file__), "..", "README.md")
    _update_bench_speedup(readme_path, pct_fortran, mult_scipy)
